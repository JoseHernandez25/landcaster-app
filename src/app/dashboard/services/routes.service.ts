...
get(paginateParameters: any) {
  ...
  return this.http.get<Pagination>(`${environment.apiUrl}/routes`, { headers, params });
}

store(area: any) {
  const headers = new HttpHeaders()
    .set('Authorization', `Bearer ${this.authService.token()} ` || '');
  return this.http.post<any>(`${environment.apiUrl}/routes`, area, { headers });
}
...
update(id: number, area: any) {
  const headers = new HttpHeaders()
    .set('Authorization', `Bearer ${this.authService.token()} ` || '');
  return this.http.put<any>(`${environment.apiUrl}/routes/${id}`, area, { headers });
}
...
delete(id: number) {
  const headers = new HttpHeaders()
    .set('Authorization', `Bearer ${this.authService.token()} ` || '');
  return this.http.delete<any>(`${environment.apiUrl}/routes/${id}`, { headers });
}
}
